...
case $FREQUENCY in 
  ...
  SEMIANNUAL ) THIS_MONTH=`date +%m`
               ...
               if [[ $THIS_MONTH -ge $N_SECOND ]] && [[ $THIS_MONTH -le 12 ]]; then
               ...
               elif [[ $THIS_MONTH -lt $N_FIRST ]]; then
...
for STP in $STPS; do
   ...
   $CLONE_BY_DATE -y -g --channels=${SOURCE} ${TARGET} --username $SATELLITE_LOGIN --password $SATELLITE_PASSWORD --to_date=${END_DATE} --blacklist="${BLSTRING}"
   ...
   for CC in $CCL; do
      ...
      if [[ -n $CCS ]]; then
         ...
         $CLONE_BY_DATE -y -g -l $CCS $CC -a $SOURCE $TARGET --username $SATELLITE_LOGIN --password $SATELLITE_PASSWORD --to_date=${END_DATE}
      else
